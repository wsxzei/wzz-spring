...
import cn.wzz.springframework.context.annotation.ClassPathBeanDefinitionScanner;
...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
    ...
    protected void doLoadBeanDefinition(InputStream inputStream) throws ClassNotFoundException {
        ...
        Element componentScan = null;
        for(int i = 0; i < nodes.getLength(); i++){
            if(!(nodes.item(i) instanceof Element)) continue;
            if(nodes.item(i).getNodeName().equals("context:component-scan")){
                componentScan = (Element) nodes.item(i);
                break;
            }
        }
    ...
    /**
     * 扫描base-package中指定的包路径, ','作为分隔符; 然后将包路径传入doScan方法, 方法执行如下操作:
     * 解析出@Component标记的类对应的bean定义集合, 并设置scope和获取beanName
     * 最后将bean定义注册到BeanDefinitionRegistry
     */
