...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext {
    ...
    public void refresh() throws BeansException {
        ...
        // 添加: 为beanFactory设置ConversionService, 用于在bean创建的注入属性过程中完成数据类型的转换
    ...
    // 将转换服务对象注册到bean工厂中, 后续bean注入属性时进行数据类型转换
    private void registerConversionService(ConfigurableListableBeanFactory beanFactory){
        if(beanFactory.containsBean("conversionService")) {
            ConversionService conversionService = (ConversionService) beanFactory.getBean("conversionService");
