...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
    ...
    protected Object doCreateBean(String beanName, BeanDefinition beanDefinition, Object... args){
        ...
        try{
            ...
            // 处理循环依赖, 将实例化后的bean对象提前暴露到三级缓存中
            if(beanDefinition.isSingleton()){
                final Object finalObject = bean;
                addSingletonFactory(beanName, ()->
                        getEarlyBeanReference(beanName, beanDefinition, finalObject));
            }
            ...
            throw new BeansException("Instantiation of bean failed; " + e.getMessage(), e);
        }
        ...
        if(beanDefinition.isSingleton()) {
            // 如果是代理对象, 此时会从三级缓存的工厂对象获取需要暴露的对象;
        ...
        try {
            ...
            for (PropertyValue pv : propertyValues.getPropertyValues()) {
                ...
                if (propertyValue instanceof BeanReference) {
                    ...
                    // 因为要获取bean对象的字段类型(包含私有属性), 所以必须使用getDeclaredField方法, 入参传入bean对象的原始类型
                    ...
                    // 获取字段propertyName的类型
                    Class<?> targetType = beanClass.getDeclaredField(propertyName).getType();
                ...
                // 测试阶段使用如下代理进行属性注入, 这样能检验属性类型转换是否成功
