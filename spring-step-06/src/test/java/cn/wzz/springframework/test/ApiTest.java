 ...
 *
 * 06版本:
 *      6.1 不使用上下文, 测试BeanFactoryPostProcessor和BeanPostProcessor扩展机制
 *          分别在(bean定义注册后, 实例化前)与(bean实例化后)执行对bean信息的修改
 *      6.2 使用应用上下文, Spring.xml需要包含MyBeanFactoryPostProcessor信息和MyBeanPostProcessor类的bean信息
 *          应用上下文的refresh注册所有bean定义后：
 *              会使用MyBeanFactoryPostProcessor修改bean定义; 实例化MyBeanPostProcessor并进行注册
 * */
public class ApiTest {
    ...
    public void test_BeanFactoryPostProcessorAndBeanPostProcessor(){
        ...
        // 3. BeanDefinition实例化前, 修改Bean定义
        MyBeanFactoryPostProcessor beanFactoryPostProcessor = new MyBeanFactoryPostProcessor();
        beanFactoryPostProcessor.postProcessBeanFactory(beanFactory);

        // 4. 将BeanPostProcessor注册到容器中
        beanFactory.getBeanPostProcessors().add(new MyBeanPostProcessor());
